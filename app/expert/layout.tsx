...
import { LayoutDashboard, ClipboardList, CheckCircle, PlusCircle, UserCircle, Bell, TrendingUp, HeartPulse, BookUser } from 'lucide-react';
...
import ExpertDashboardPage from '@/app/expert/page'; // On importe le dashboard principal
import ValidationListPage from '@/app/expert/validation/page'; // Page de liste de validation
import CreateCase from '@/components/expert/Create'; // Formulaire de création
import Analytics from '@/components/expert/Analytics';
import Library from '@/components/dashboard/Library';

// Type pour les vues possibles
type ExpertView = 'dashboard' | 'validation' | 'analytics' | 'create';

// --- SIDEBAR MODIFIÉE ---
// Elle reçoit maintenant l'état de la vue active et la fonction pour le changer
const Sidebar = ({ activeView, setActiveView }: { activeView: ExpertView, setActiveView: (view: ExpertView) => void }) => {
  ...
  const navItems = [
    ...
    // NOTE: Le lien vers la Bibliothèque de Cas a été enlevé car il sort du layout expert.
    // Il pourrait être ajouté en tant que <Link> si vous souhaitez conserver la navigation par page pour celui-ci.
  ];
    ...
    <aside className="w-64 flex-shrink-0 bg-[#052648] text-white flex flex-col">
      ...
      <nav className="flex-1 p-4 space-y-2">
        {navItems.map(item => {
          ...
          return (
            // On remplace <Link> par <button> pour changer l'état local
            <button
              key={item.id}
...
// --- HEADER MODIFIÉ ---
// Il reçoit maintenant le profil de l'utilisateur
const Header = () => {
    const { profile } = useAuth(); // Récupère le profil de l'utilisateur connecté

    const getShortName = (name: string | undefined) => {
        if (!name) return "Expert";
        return name.split(' ')[0]; // Retourne juste le premier mot (prénom)
    }
...
export default function ExpertLayout({ children }: { children: ReactNode }) {
  // On gère l'état de la vue active ici
  const [activeView, setActiveView] = useState<ExpertView>('dashboard');

  // Fonction pour afficher le bon composant en fonction de la vue active
  const renderContent = () => {
    switch (activeView) {
      case 'dashboard':
        return <ExpertDashboardPage />;
      case 'validation':
        // Note: Ici, nous ne pouvons plus afficher de page dynamique [caseId].
        // Cet affichage montre uniquement la liste. 
        // La gestion des détails nécessiterait une étape de plus (non demandée).
  ...
  return (
    <CaseProvider>
      ...
      <div className="min-h-screen flex bg-slate-100 font-sans">
        ...
        {/* On passe l'état et le setter à la Sidebar */}
        <Sidebar activeView={activeView} setActiveView={setActiveView} />
        
        <div className="flex-1 flex flex-col h-screen">
          <Header />
          <main className="flex-1 p-8 overflow-y-auto">
            {/* On appelle la fonction de rendu au lieu de `children` */}
            {renderContent()}
          </main>
        </div>
      </div>
