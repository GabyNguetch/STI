...
// CORRECTION : Ajout de 'lib' ici pour autoriser cette valeur dans le state
...
<nav className="flex-1 p-4 space-y-2">
    {navItems.map(item => {
      ...
      return (
        <button
          key={item.id}
          // Cast explicite si nécessaire pour rassurer TS, mais ici string match l'id
...
<Sidebar activeView={activeView} setActiveView={setActiveView} />
    ...
    <div className="flex-1 flex flex-col h-screen">
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        {/* CORRECTION : On affiche le contenu généré par le switch */}
        {renderContent()}
      </main>
    </div>
  </div>
